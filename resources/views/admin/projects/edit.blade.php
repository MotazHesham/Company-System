            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $project->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('developers') ? 'is-invalid' : '' }}" name="developers[]" id="developers" multiple required>
                    @foreach($developers as $id => $developer)
                        <option value="{{ $id }}" {{ (in_array($id, old('developers', [])) || $project->developers->contains($id)) ? 'selected' : '' }}>{{ $developer }}</option>
                    @endforeach
                </select>
